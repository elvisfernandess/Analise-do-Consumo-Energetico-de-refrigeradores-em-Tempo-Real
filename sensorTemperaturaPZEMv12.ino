...
void loop() {
  ...
  float power = pzem.power() - 0.40; //pega a potência medida do sensor de energia 
  //delay(100);
  float current = (power/voltage); //pega a corrente medida do sensor de energia 
  ...
  Serial.print(abs(current), 4); // apenas valores positivos com 4 casas decimais
  Serial.println(" A");

  Serial.print("Power: "); //exibe valores lidos de potencia ativa no console Serial
  Serial.print(abs(power),4); // apenas valores positivos com 4 casas decimais
  Serial.println(" W");
